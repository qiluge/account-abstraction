 ...
 */
contract SponsorDepositPaymaster is BasePaymaster {
    ...
    // sponsor => fee token => user => quota
    mapping(address => mapping(IERC20 => mapping(address => uint))) public sponsors;
    ...
    /// @notice reset quota so that sponsor could revoke allowance
    function setSponsors(IERC20 token, address[] memory users, uint[] memory quotas) public {
        require(users.length == quotas.length, 'ill param');
        for (uint i = 0; i < users.length; i++) {
            sponsors[msg.sender][token][users[i]] = quotas[i];
        }
    }
    ...
    internal view override returns (bytes memory context, uint256 validationData) {
        ...
        require(paymasterAndData.length == 20 + 20 || paymasterAndData.length == 20 + 20 + 20,
            "DepositPaymaster: paymasterAndData must specify token");
        IERC20 token = IERC20(address(bytes20(paymasterAndData[20 : 40])));
        address account = userOp.getSender();
        uint256 maxTokenCost = getTokenValueOfEth(token, maxCost);
        uint256 gasPriceUserOp = userOp.gasPrice();
        if (paymasterAndData.length == 40) {// pay by self
            ...
            address sponsor = address(bytes20(paymasterAndData[40 :]));
            require(unlockBlock[sponsor] == 0, "DepositPaymaster: sponsor deposit not locked");
            require(balances[token][sponsor] >= maxTokenCost, "DepositPaymaster: sponsor deposit too low");
            require(sponsors[sponsor][token][account] >= maxTokenCost, "DepositPaymaster: exceed sponsor's allowance");
            // replace account so that we can deduct fee from sponsor when _postOp
            account = sponsor;
        }
